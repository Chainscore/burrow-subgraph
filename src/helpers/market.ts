...
import { Market, MarketDailySnapshot, MarketHourlySnapshot, InterestRate } from '../../generated/schema';
import { BI_ZERO, BD_ZERO, ADDRESS_ZERO, NANOSEC_TO_SEC, NANOS_TO_DAY, NANOS_TO_HOUR } from '../utils/const';
...
export function getOrCreateMarketDailySnapshot(
	...
	if(!snapshot){
		...
		snapshot.timestamp = BigInt.fromU64(NANOSEC_TO_SEC(receipt.block.header.timestampNanosec))
		// supply rate
...
export function getOrCreateMarketHourlySnapshot(
	...
	const hourId = (NANOS_TO_HOUR(receipt.block.header.timestampNanosec)).toString()
